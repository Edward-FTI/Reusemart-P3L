...
const Dashboard = () => {
    ...
    const handleAddToCart = () => {
        if (selectedBarang) {
            const isItemInCart = cartItems.some(item => item.id === selectedBarang.id);

            if (isItemInCart) {
                toast.info(`${selectedBarang.nama_barang} sudah ada di keranjang.`);
            } else {
                setCartItems((prevItems) => [...prevItems, selectedBarang]);
                toast.success(`${selectedBarang.nama_barang} berhasil ditambahkan ke keranjang!`);
            }
            handleModalClose();
        }
    };
    ...
    return (
        <div>
            ...
            <div className="modal fade" id="detailBarangModal" tabIndex="-1" aria-labelledby="detailBarangModalLabel" aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="detailBarangModalLabel">{selectedBarang?.nama_barang}</h5>
                            <button
                                type="button"
                                className="btn-close"
            ...
            <div className={`offcanvas offcanvas-end ${showCartOffcanvas ? 'show' : ''}`} tabIndex="-1" id="cartOffcanvas" aria-labelledby="cartOffcanvasLabel">
                <div className="offcanvas-header">
                    <h5 className="offcanvas-title" id="cartOffcanvasLabel">Keranjang Belanja ({cartItems.length})</h5>
                    <button type="button" className="btn-close" data-bs-dismiss="offcanvas" aria-label="Close" onClick={toggleCartOffcanvas}></button>
                </div>
                            ...
                            {cartItems.map((item) => (
                                <li key={item.id} className="list-group-item d-flex justify-content-between align-items-center">
                                    <div className="d-flex align-items-center">
                                        ...
                                        />
                                        <div>
                                            <h6 className="mb-0">{item.nama_barang}</h6>
                                            <small className="text-muted">Rp {item.harga_barang}</small>
                                        </div>
                                    </div>
                                    <button
