    ...
    public function indexHunterHistory()
    {
        try {
            ...
            if (!$user || strtolower($user->role) === 'Hunter') {
                return response()->json([
                    'status' => false,
                    'message' => 'Hunter belum login',
                ], 401);
            }

            // Cari data pegawai berdasarkan email dari user login
            ...
            // Ambil semua transaksi penjualan yang berkaitan dengan barang hunter ini
            $transaksiIds = DB::table('detail_transaksi_penjualans')
